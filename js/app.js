document.addEventListener('DOMContentLoaded', function() {
    ...
    generarLink.addEventListener('click', () => {
        ...
        const link = `${url}?show-replies=${showReplies.checked}&dnt=${dnt.checked}&theme=${theme.checked? 'dark' : 'light'}&chrome=${chromeValues.slice(0,-1)}&width=${width.value||0}&height=${height.value||0}&tweet-limit=${tweetLimit.value}`
        getLink.value = link;
    })
});
